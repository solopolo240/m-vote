...
LOCAL_API_URL = 'https://api.telegram.org/bot'
        ...
        for query in queries:
            ...
            for video in candidates:
                ...
                if await self.send_to_telegram(path, audio_title, video):
                    # Delete only the bot's "Searching..." status on first success
                    if not cleared and chat_id and status_msg_id:
                        await self._delete_messages(chat_id, status_msg_id)
                        cleared = True
...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Status message — deleted when first audio arrives
...
async def keyword_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "list":
        ...
        msg += "🔹 Defaults: mezmur, ዝማሬ, ዘማሪ, ዲያቆን, orthodox, ኦርቶዶክስ, tewahedo, ተዋሕዶ, church, ቤተክርስቲያን\n"
